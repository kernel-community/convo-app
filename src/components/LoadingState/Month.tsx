...
const CalendarLoadingState = () => {
  return (
    <div className="relative isolate w-full overflow-hidden rounded-lg">
      <div className="absolute h-full w-full -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-muted/5 via-muted/30 to-muted/5" />
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-7 gap-1">
            {Array.from({ length: 7 }).map((_, i) => (
              <div key={i} className="h-6 rounded-md bg-muted/20" />
            ))}
          </div>
